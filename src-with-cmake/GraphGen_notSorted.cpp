...
void fill_up_edge_vector(
        ...
        const double RMAT_a, const double RMAT_b, const double RMAT_c, const bool allowEdgeToSelf,
        const bool allowDuplicateEdges, const bool directedGraph
) {
    ...
    generate_edges(std::ref(rec), std::ref(edgeVector), RMAT_a, RMAT_b, RMAT_c, directedGraph, allowEdgeToSelf,
                   std::ref(dis), std::ref(gen), std::ref(throwAwayEdgesIndices));
    ...
    if (!allowDuplicateEdges) {
        ...
        do {
            ...
            if (!throwAwayEdgesIndices.empty())
                generate_edges(std::ref(rec), std::ref(edgeVector), RMAT_a, RMAT_b, RMAT_c, directedGraph,
                               allowEdgeToSelf, std::ref(dis), std::ref(gen), std::ref(throwAwayEdgesIndices));
    ...
    if (USE_A_MUTEX_TO_CONTROL_WRITE_TO_FILE_INSTEAD_OF_CONCURRENT_QUEUES) {
        ...
        auto eachThreadGenEdgesUsingMutexFunc = [&](unsigned int puId) {
            ...
            for (unsigned int recIdx = puId; recIdx < squares.size(); recIdx += nWorkerThreads) {
                ...
                fill_up_edge_vector(std::ref(rec), std::ref(edgeVector), std::ref(dis), std::ref(gen), RMAT_a, RMAT_b,
                                    RMAT_c, allowEdgeToSelf, allowDuplicateEdges, directedGraph);
                {
                    std::lock_guard<std::mutex> guard(writeMutex);
//					printEdgeGroup( std::ref(edgeVector), outFile );
                    printEdgeGroup(std::ref(edgeVector), outFileEdges, outFileEdgeNum);
...
//			printEdgeGroupNoFlush( poppedEV, outFile );
            printEdgeGroup(poppedEV, outFileEdges, outFileEdgeNum);
