...
void
ShatterSquare(std::vector<Square> &square, const double a, const double b, const double c, const unsigned int index,
              const bool directedGraph) {

    std::srand(std::time(0));

    // Noise for selecting a, b, c to cut square.
    auto noise_a = static_cast<double>(std::rand() - (RAND_MAX / 2)) / (RAND_MAX *
                                                                        100.0);    // Very small random noises. Maybe can be implemented in a better way. Up to one percent for each parameter.
    ...
    auto noise_a_edge_share = static_cast<double>(std::rand() - (RAND_MAX / 2)) / (RAND_MAX *
                                                                                   100.0);    // Very small random noises. Maybe can be implemented in a better way. Up to one percent for each parameter.
    ...
    for (unsigned int i = 0; i < 4; ++i)
        square.push_back(srcRect.Get_part(i, noise_a + a, noise_b + b, noise_c + c, noise_a_edge_share + a,
                                          noise_b_edge_share + b, noise_c_edge_share + c));
    ...
    if (!directedGraph) {
        if (srcRect.get_X_end() ==
            srcRect.get_Y_end()) {    // If the source rectangle is on the main diagonal, we should throw away the 2nd part (b).
            square.at(square.size() - 2).setnEdges(square.at(square.size() - 2).getnEdges() +
                                                   square.at(square.size() -
                                                             3).getnEdges());    // We add the edges from b part to c part.
...
unsigned long long
genEdgeIndex_FP(unsigned long long startIdx_ull, unsigned long long endIdx_ull, const double a, const double b_or_c,
...
void printEdgeGroup(std::vector<Edge> &edges, std::ofstream &outFile) {
    fprintf(stderr, "edge groups %zu\n", edges.size());
    std::cout << "edge groups " << edges.size() << std::endl;
    printEdgeGroupNoFlush(edges, outFile);
    outFile.flush();
}

void printEdgeGroup(std::vector<Edge> &edges, Edge *output_edges, size_t &output_edge_num) {
    memcpy(output_edges + output_edge_num, &edges.front(), edges.size() * sizeof(Edge));
    output_edge_num += edges.size();
}
...
bool edgeOverflow(std::vector<Square> &sqaures) {
    for (auto &rec : sqaures)
        if (3 * rec.getnEdges() >= (rec.get_X_end() - rec.get_X_start()) * (rec.get_Y_end() -
                                                                            rec.get_Y_start()))    // 3 is experimental. It tells if the size of square is less than 3 times the number of edges, square shouldn't be shattered.
            return true;
...
void generate_edges(Square &squ,
    ...
    auto applyCondition = directedGraph || (squ.get_H_idx() <
                                            squ.get_V_idx()); // true: if the graph is directed or in case it is undirected, the square belongs to the lower triangle of adjacency matrix. false: the diagonal passes the rectangle and the graph is undirected.
    ...
    for (unsigned long long edgeIdx = 0; edgeIdx < nEdgesToGen;) {
        unsigned long long h_idx = genEdgeIndex_FP(squ.get_X_start(), squ.get_X_end(), RMAT_a, RMAT_c, std::ref(dis),
                                                   std::ref(gen));
        unsigned long long v_idx = genEdgeIndex_FP(squ.get_Y_start(), squ.get_Y_end(), RMAT_a, RMAT_b, std::ref(dis),
                                                   std::ref(gen));
        if ((!applyCondition && h_idx > v_idx) ||
            (!allowEdgeToSelf && h_idx == v_idx)) // Short-circuit if it doesn't pass the test.
            continue;
